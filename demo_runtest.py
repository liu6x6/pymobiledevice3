...
host = 'fde7:56e3:5a76::1'  # randomized
port = 51631  # randomized
...
def test_installservice(rsd):
    with InstallcoordinationProxyService(rsd) as install_service:
        ...
        sleep(1)
        print("=========================")
        re = install_service.service.send_request({'RequestVersion': XpcUInt64Type(1), 'BundleID': 'com.apple.test.WebDriverAgentRunner-Runner', 'ProtocolVersion': XpcUInt64Type(1), 'RequestType': XpcUInt64Type(4)})
        ...
        # install_service.service._open_channel(upload_stream, flags=flags)
        ...
        wrapper = XpcWrapper.parse(wrapperdata)
        install_service.service._send_frame(
            DataFrame(stream_id=upload_stream, data=wrapperdata))
        while True:
            sleep(10)
            print(install_service.service_name)
            re = install_service.service.send_request({'RequestVersion': XpcUInt64Type(1), 'BundleID': 'com.apple.test.WebDriverAgentRunner-Runner', 'ProtocolVersion': XpcUInt64Type(1), 'RequestType': XpcUInt64Type(4)})
...
with RemoteServiceDiscoveryService((host, port)) as rsd:
    ...
    #         "AssetStreamFD": FileTransferType(0xffffffff),
